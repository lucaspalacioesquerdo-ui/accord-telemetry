...
function kpiStatus(v: number | null, warnAt: number, badAt: number, dir: 'up' | 'down' = 'up') {
    ...
    if (dir === 'up') {
          if (v >= badAt) return 'bad' as const
          if (v >= warnAt) return 'warn' as const
...
const S: Record<string, React.CSSProperties> = {
    ...
    main: { flex: 1, overflowY: 'auto' as const, padding: '28px 32px' },
    sectionTitle: {
          fontFamily: "'IBM Plex Mono', monospace",
          fontSize: 9, letterSpacing: '2.5px', textTransform: 'uppercase' as const,
          color: 'var(--muted)', marginBottom: 12,
    },
    kpiGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(155px,1fr))', gap: 10 },
  ...
  function SidebarItem({ s, i }: { s: LogSession; i: number }) {
        ...
        return (
                <div
                        ...
                        >
                  {isActive && <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 2, background: 'var(--accent)', borderRadius: '0 2px 2px 0' }} />}
                ...
                <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                  ...
                  {/* MAIN */}
                        ...
                        <div>
                          ...
                          {/* Temperatura */}
                                      <div style={S.section}>
                                                      <div style={S.sectionTitle}>Temperatura</div>div>
                                                      <div style={S.kpiGrid}>
                          ...
                          {/* Mistura */}
                                      <div style={S.section}>
                                                      <div style={S.sectionTitle}>Mistura & Fuel Trim</div>div>
                                                      <div style={S.kpiGrid}>
                          ...
                          {/* Consumo */}
                                      <div style={S.section}>
                                                      <div style={S.sectionTitle}>Consumo & Distância</div>div>
                                                      <div style={S.kpiGrid}>
                          ...
                          {active.lng_accel_max != null && (
                                          <div style={S.section}>
                                                            <div style={S.sectionTitle}>Aceleração Longitudinal</div>div>
                                                            <div style={S.kpiGrid}>
                          ...
                          {/* Elétrico */}
                                      <div style={S.section}>
                                                      <div style={S.sectionTitle}>Elétrico & Status</div>div>
                                                      <div style={S.kpiGrid}>
                          ...
                          {/* Diagnóstico */}
                                      <div style={S.section}>
                                                      <div style={S.sectionTitle}>Diagnóstico Automático</div>div>
                                                ...
                                                const isNew = dbSessions.some(d => d.name === s.name) || localSessions.some(l => l.name === s.name)
                                                                        return (
                                                                                                  <tr key={s.name} onClick={() => { setActiveIdx(i); setTab('overview') }}
                                                                                                                              style={{ borderBottom: '1px solid var(--border)', background: isNew ? 'rgba(56,189,248,0.03)' : 'transparent', cursor: 'pointer', transition: 'background 0.12s' }}>
